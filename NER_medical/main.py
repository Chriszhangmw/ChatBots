...
data_path = './data'
train_input, eval_input, test_input = get_data(data_path,bert_vocab_file,max_len)


def train():
    model = Ner_Model(bert_root,data_path,model_save_path,batch_size,max_len,lr,keep_prob)
    with tf.Session() as sess:
        writer = tf.summary.FileWriter('./tf_log/', sess.graph)
        # saver = tf.train.Saver()
        sess.run(tf.global_variables_initializer())
        data_loader = Data_loader(train_input, batch_size)
...
def test():
    data_loader = Data_loader(test_input, batch_size)
    saver = tf.train.import_meta_graph('./model/ner.model.meta')
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
